...
def send_feishu_alert(service_name, status_desc):
    ...
    payload = {
        "msg_type": "text",
        "content": {
            "text": "🚨 JM 雷达告警：服务 [{service_name}] 状态异常！" +
            "\n当前状态：{status_desc}\n检查时间：{time.ctime()}"
        },
    }
    try:
            ...
            for name, url in TARGETS.items():
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        status_text = "⚠️ 状态码异常: {resp.status_code}"
                        current_state = "Error"
                ...
                if (
                    last_known_status.get(name) == "Normal"
...
def index():
    ...
    for name, status in service_status_cache.items():
        ...
        bg_color = (
            ...
            if "✅" in status
            else "bg-red-100 border-red-500 text-red-700"
        )
        status_cards += f"""
        ...
        <body class="bg-gray-50 p-8">
            <div class="max-w-4xl mx-auto text-center">
                <header class="mb-10">
                    ...
                    🚀 JM 全网监控雷达
                    <span class="text-blue-600">v2.5</span></h1>
